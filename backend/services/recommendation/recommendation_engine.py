...
against the full O*NET career database.
"""
import numpy as np
...
def _get_career_embeddings() -> tuple[list[dict], np.ndarray]:
    """Pre-compute embeddings for all careers. Cached after first call."""
    careers = load_career_database()
    print(f"Pre-computing embeddings for {len(careers)} careers...")
    texts = [", ".join(c["required_skills"][:20]) for c in careers]  # cap at 20 skills
...
def recommend_careers(parsed_profile: dict, top_n: int = 5) -> list[dict]:
    """
    Match user profile against O*NET careers using cosine similarity.
    ...
    # Get top indices
    top_indices = np.argsort(scores)[::-1][:top_n * 3]  # get extra for filtering
    ...
    for idx in top_indices:
        ...
        results.append({
            ...
            "skill_gap": skill_gap[:8],  # top 8 missing skills
...
def _compute_skill_gap(user_skills: list[str], required_skills: list[str]) -> list[str]:
    ...
    for req in required_skills:
        req_lower = req.lower()
        # Check if any user skill is a substring match
